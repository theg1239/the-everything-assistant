  ...
  try {
    ...
    try {
      ...
      const companies = await page.evaluate(() => {
        ...
        const companyTable = tables.find(table => 
          table.textContent?.includes('Company') && 
          table.textContent?.includes('Package')
        );
      ...
      const recentOffers = await page.evaluate(() => {
        const tables = Array.from(document.querySelectorAll('[data-testid="stTable"]'));
        const offersTable = tables.find(table => 
          table.textContent?.includes('Student') && 
          table.textContent?.includes('Offer')
        );
      ...
      if (company) {
        ...
        placementData.companies = placementData.companies.filter(
          c => c.name.toLowerCase().includes(companyLower)
        );
        placementData.recentOffers = placementData.recentOffers.filter(
          o => o.company.toLowerCase().includes(companyLower)
        );
      }
