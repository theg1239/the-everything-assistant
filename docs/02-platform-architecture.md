...
## 4. Models, Rate Limiting & OpenAI

### Rate-Limited AI Client (`lib/rate-limited-ai.ts`)

- Wraps only `@ai-sdk/openai` with:
  ...
  - Helper methods to stream text, generate objects, embed vectors, generate images, expose OpenAI hosted tools, and produce usage stats.
- `getRateLimitedAI('openai')` exports the memoized client; `rateLimitedAI.openai` provides the app-facing shorthand.
- Workloads are tiered across GPT-5.6 Sol, Terra, and Luna. Embeddings use `text-embedding-3-large`, and image generation uses `gpt-image-2`.

### Environment Guardrails (`lib/env-config.ts`)

- Validates required env vars (OpenAI keys, Redis credentials, admin email).
...
- `/api/whatsapp-bot` ingests WhatsApp or Discord payloads (distinguished via the `source` field). It creates “bot users” (fake emails like `whatsapp-XXXX@whatsapp-bot.local`) and routes the entire message list through the same `rateLimitedAI.openai.streamText` path used by the web UI.
...
- **Env Vars**: Keep `.env.example` current. Minimum for local dev: Google OAuth keys, `DATABASE_URL`, Upstash Redis, `UPSTASH_REDIS_REST_TOKEN`, VTOP proxy URL, `OPENAI_API_KEY`, `RESEND_API_KEY`, `RATE_LIMIT_ADMIN_EMAIL`. `PARALLEL_API_KEY` enables the retained Parallel search/extract tools.
