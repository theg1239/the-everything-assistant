...
export function recognizeCoursesInBackground(text: string): CourseContext {
  ...
  try {
    ...
    const words = text.toUpperCase().split(/\W+/).filter(word => word.length >= 2)
    words.forEach(word => {
      if (COURSE_ACRONYMS[word]) {
        ...
        if (!recognitions.some(r => r.original.toUpperCase() === word)) {
          recognitions.push({
            ...
            confidence: courses.length === 1 ? 0.85 : 0.70,
            type: 'acronym'
          })
        }
    ...
    const phrases = text.toLowerCase().split(/[.!?;]/).filter(phrase => phrase.trim().length > 10)
    ...
    const courseKeywords = [
      'syllabus', 'curriculum', 'assignment', 'exam', 'cat1', 'cat2', 'fat', 'quiz',
      'professor', 'faculty', 'lecture', 'lab', 'practical', 'theory', 'credits',
      'semester', 'course', 'subject', 'module', 'unit', 'chapter'
    ]
    
    const hasCourseMentions = courseKeywords.some(keyword => 
...
export function getCourseSuggestions(partial: string, limit: number = 5): Array<{code: string, name: string}> {
