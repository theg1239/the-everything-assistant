...
const TimetableGrid: React.FC<TimetableGridProps> = ({ timetable, schema }) => {
  const { days, timeHeaders, slotGridMap, rowCount } = useMemo(() => {
    ...
    const timeHeaders = [...new Set(allSlots.filter(s => !s.lunch).map(s => s.start))].sort(
    ...
    <div className="bg-gray-800 p-4 rounded-lg shadow-inner">
      ...
      >
        ...
        {days.map((day, i) => (
          <div
            key={day}
            className="text-center font-bold text-gray-300 p-2"
        ...
        {timeHeaders.map((time, i) => (
          <div
            key={time}
            className="text-right text-xs text-gray-400 pr-2 pt-1"
