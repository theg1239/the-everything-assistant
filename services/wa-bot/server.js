...
class WABotServer {
  ...
  setupRoutes() {
    ...
    this.app.get('/api/whatsapp/conversations', (req, res) => {
      try {
        const conversations = Array.from(this.whatsappService?.activeChats.entries() || []).map(
          ([phoneNumber, data]) => ({
            phoneNumber,
            ...data,
          })
  ...
  async setupWhatsApp() {
    try {
      ...
      this.whatsappService = new WhatsAppService({
        ...
        puppeteerArgs: (
          process.env.WHATSAPP_PUPPETEER_ARGS || '--no-sandbox,--disable-setuid-sandbox'
        ).split(','),
      })
  ...
  setupWhatsAppHandlers() {
    ...
    this.whatsappService.on('message', messageData => {
      ...
      console.log(
        `${messageType} message: ${messageData.fromName} - ${messageData.body.substring(0, 50)}...`
      )
    })
  }
