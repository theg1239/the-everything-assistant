 ...
 * 
 * Run with: node test/scrapers/papers-api-test.mjs
 */

import { scrapePapersCodeChef } from '../../lib/scrapers/papers-codechef.js'
import { scrapeVITPaperVault } from '../../lib/scrapers/vit-papervault.js'
...
async function testPapersAPIs() {
  ...
  for (const testCourse of testCourses) {
    ...
    try {
      ...
      console.error('❌ CodeChef API test failed:', error.message)
    }
    ...
    try {
      ...
      console.error('❌ PaperVault API test failed:', error.message)
    }
  ...
  console.log('  ✓ No "falling back to browser scraping" messages in logs')
