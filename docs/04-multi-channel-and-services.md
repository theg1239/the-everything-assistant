...
- Components:
  ...
  - `actions/uploadPaper.ts` (Server Action hooking to Cloudinary, `pdf-lib`, sharp, and OpenAI structured vision output).
...
- Upload pipeline:
  ...
  3. Feed the PDF into `generateObject` (GPT-5.6 Luna) with `PaperMetadataSchema`.
