...
async function tryVITVaultListAPI(
  courseCode: string,
  examType?: string,
  year?: string
) {
  try {
    ...
    const filtered = all.filter((p) => {
      ...
      const sameYear =
        !year ||
        new Date(p.paperDate).getUTCFullYear().toString() === year;
...
async function tryBrowserScraping(
  ...
  let browser;
  try {
    // POINT CHROMIUM AT /var/task/node_modules/@sparticuz/chromium/bin
    const binDir = path.join(
      "/var/task/node_modules/@sparticuz/chromium/bin"
    );
    let execPath: string;
    try {
      execPath = await chromium.executablePath(binDir);
    } catch {
    ...
    const papers = await page.evaluate(
      (courseCode, examType, year) => {
        ...
        els.forEach((el) => {
          ...
          const okCourse =
            tl.includes(cc) ||
            tl.includes(cc.replace(/(\d+)/, " $1"));
          const okExam =
            !examType || tl.includes(examType.toLowerCase());
          const okYear =
            !year || tl.includes(year) || (href.includes(year) as any);
