...
export async function parsePlacementData(
  ...
  console.log('--- SERVER: Parsing Placement Data ---');
  // eslint-disable-next-line no-console
  console.log('Raw data received for parsing:', JSON.stringify(rawData, null, 2));
  try {
...
`,
    ...
    // eslint-disable-next-line no-console
    console.log('--- SERVER: Successfully parsed placement data ---', JSON.stringify(result.object, null, 2));
    return result.object;
  } catch (error) {
    ...
    if (directToolCall) {
      ...
      if (tool && typeof tool.execute === 'function') {
        try {
          ...
          if (
            directToolCall.toolName === 'getPlacementInfo' &&
            result.success &&
            'data' in result &&
            result.data
          ) {
            // eslint-disable-next-line no-console
            console.log('--- SERVER: Handling getPlacementInfo direct tool call ---');
            // eslint-disable-next-line no-console
            console.log('Tool result from scraper:', JSON.stringify(result, null, 2));
            try {
              const userContext =
                messages && messages.length > 0
                  ? messages
                      ...
                      .join(' | ')
                  : '';
              const parsedData = await parsePlacementData(result, userContext, session.user.id)
              // eslint-disable-next-line no-console
              console.log('Parsed data from LLM:', JSON.stringify(parsedData, null, 2));
              ...
              // eslint-disable-next-line no-console
              console.log('--- SERVER: Final enriched result ---', JSON.stringify(enrichedResult, null, 2));
              const assistantResponse =
                enrichedResult.summary ||
                'Successfully retrieved placement data.';
              ...
              console.error('Failed to parse Placement data:', parseError);
              return new Response(JSON.stringify({ success: false, error: 'Failed to parse placement data' }), {
                status: 500,
                headers: { 'Content-Type': 'application/json' },
              });
            }
