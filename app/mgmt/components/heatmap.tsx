...
export default function Heatmap({ rows = [], max = 0 }: { rows: HeatmapRow[]; max: number }) {
  ...
  const [tip, setTip] = useState<{ x: number; y: number; day: string; hour: number; tokens: number } | null>(null)
    ...
    <div className="mt-2">
      ...
      <div ref={containerRef} className="overflow-auto">
        ...
        <div className="flex items-center gap-2 text-[11px] text-muted-foreground mb-1">
          ...
          <div className="flex-1 grid grid-cols-24 gap-[2px]">
            {Array.from({ length: 24 }).map((_, i) => (
              <div key={i} className="text-center text-[11px]">{i}</div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-1">
          {rows.length === 0 && <div className="text-xs text-muted-foreground">no heatmap rows</div>}
      ...
      {tip && (
        ...
        >
          <div className="font-medium">{tip.day} • {tip.hour}:00</div>
