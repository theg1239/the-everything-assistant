...
if (!NEW_URL) { console.error("Error: NEWDATABASE_URL must be set in .env"); process.exit(1); }

const PRISMA_SCHEMA = process.env.PRISMA_SCHEMA || path.resolve(__dirname, "../prisma/schema.prisma");
...
function runPSQLFile(url, sql, flags = "-At") {
  const tmp = path.join(os.tmpdir(), `psql_${Date.now()}_${Math.random().toString(36).slice(2)}.sql`);
...
function execPSQLFile(url, sql) {
  ...
  if (DRY_RUN) { console.log("(dry-run) " + cmd + "\n" + sql); try { fs.unlinkSync(tmp); } catch {} return true; }
  try { execSync(cmd, { stdio: "inherit", env: PSQL_ENV }); return true; }
  catch { return false; }
  finally { try { fs.unlinkSync(tmp); } catch {} }
}
...
function extractModels(src) {
  const models = []; let i=0;
  const isWord = ch => /[A-Za-z0-9_]/.test(ch);
  while (i < src.length) {
    const idx = src.indexOf("model", i); if (idx === -1) break;
    const before = idx ? src[idx-1] : " ", after = idx+5 < src.length ? src[idx+5] : " ";
    if (isWord(before) || !/\s/.test(after)) { i = idx+5; continue; }
    let j = idx+5; while (/\s/.test(src[j])) j++;
    let ns = j; while (/[A-Za-z0-9_]/.test(src[j])) j++;
    const name = src.slice(ns, j); while (/\s/.test(src[j])) j++;
    if (src[j] !== "{") { i = j+1; continue; }
    let k = j, d = 0, inS=false, sC=null, inL=false, inB=false;
    while (k < src.length) {
      const ch = src[k], nx = src[k+1];
      if (inL){ if (ch === "\n") inL=false; k++; continue; }
      if (inB){ if (ch==="*" && nx==="/"){ inB=false; k+=2; continue; } k++; continue; }
      if (inS){ if (ch === "\\" && nx){ k+=2; continue; } if (ch===sC){ inS=false; sC=null; } k++; continue; }
      if (ch==="/" && nx==="/"){ inL=true; k+=2; continue; }
      if (ch==="/" && nx==="*"){ inB=true; k+=2; continue; }
      if (ch==="'"||ch==='"'||ch==="`"){ inS=true; sC=ch; k++; continue; }
      if (ch==="{"){ d++; k++; continue; }
      if (ch==="}"){ d--; k++; if (d===0) break; continue; }
      k++;
    }
...
const models = rawModels.map(({ name, body }) => {
  ...
  for (const line of lines) {
    ...
    if (!m) continue;
    const fieldName = m[1], rawType = m[2], attrs = m[3] || "";
...
function topo(nodes, edges){
  const indeg=new Map(nodes.map(n=>[n,0])), adj=new Map(nodes.map(n=>[n,[]]));
  for (const [a,b] of edges){ adj.get(a).push(b); indeg.set(b,(indeg.get(b)||0)+1); }
  const q=[], out=[]; for (const n of nodes) if((indeg.get(n)||0)===0) q.push(n);
  while(q.length){ const x=q.shift(); out.push(x); for(const y of adj.get(x)){ indeg.set(y,indeg.get(y)-1); if(indeg.get(y)===0) q.push(y);} }
  return out.length===nodes.length?out:nodes;
}
const nodes = models.map(m=>m.tableName);
const edges = [];
for (const m of models) for (const f of m.fields) if (f.isRelation) {
  const refModel = f.rawType.replace(/\?$/,"").replace(/\[\]$/,"");
  const refTable = (models.find(mm=>mm.modelName===refModel)||{tableName:refModel}).tableName;
...
if (!fs.existsSync(CSV_DIR)) { console.error("CSV directory not found:", CSV_DIR); process.exit(1); }
...
function parseCsv(text) {
  ...
  while (i<text.length) {
    ...
    if (inQ) { if (ch === '"' && nx === '"'){ field+='"'; i+=2; continue; }
...
function toPgBytea(val, sourceEncoding) {
  ...
  if (/^[A-Za-z0-9+/]+={0,2}$/.test(v)) { try { return "\\x" + Buffer.from(v, "base64").toString("hex"); } catch {} }
...
(async function main() {
  ...
  for (const t of importOrder) {
    ...
    if (!target) { console.warn(`Skip: no matching target table found in Cockroach for '${t}'.`); continue; }

    const tableColsExact = getCrdbColumns(target);
    const tableColsMap = new Map(tableColsExact.map(c => [c.toLowerCase(), c]));
    if (!tableColsExact.length) { console.warn(`Skip: target table '${target}' has no columns?`); continue; }
    ...
    for (const { file } of parts) {
      ...
      if (!keep.length) { console.warn(`Skip: ${path.basename(file)} has no columns matching ${target}.`); continue; }
      ...
      if (t.toLowerCase() === "messages") {
        ...
        if (nonEmpty.length) {
          ...
          if (!execPSQLFile(NEW_URL, `\\COPY ${qid(target)} (${colListA}) FROM ${qpath(filteredA)} CSV HEADER`)) break;
          console.log(`Imported ${path.basename(file)} (non-empty subset) -> ${target}`);
          try { fs.unlinkSync(filteredA); } catch {}
        }
        ...
        if (empty.length) {
          ...
          const ok = execPSQLFile(NEW_URL, `\\COPY ${qid(target)} (${colListB}) FROM ${qpath(filteredB)} CSV HEADER`);
          execPSQLFile(NEW_URL, dropDef);
          try { fs.unlinkSync(filteredB); } catch {}
          if (!ok) break;
      ...
      const ok = execPSQLFile(NEW_URL, `\\COPY ${qid(target)} (${colList}) FROM ${qpath(filtered)} CSV HEADER`);
      try { fs.unlinkSync(filtered); } catch {}
      if (!ok) break;
  ...
  for (const [t, parts] of byTable) {
    ...
    if (!crdbHasTable(target)) { console.warn(`Skip: '${t}' CSVs exist but table '${target}' not found in Cockroach.`); continue; }
    const tableColsExact = getCrdbColumns(target);
    const tableColsMap = new Map(tableColsExact.map(c => [c.toLowerCase(), c]));
    const arrSet  = arrayColsByTable.get(t) || new Set();
    const byteSet = byteColsByTable.get(t)  || new Set();
    ...
    for (const { file } of parts) {
      ...
      const rows = parseCsv(raw); if (!rows.length) continue;
      const header = rows[0].map(h => (h ?? "").trim());
      const keep = []; for (let i=0;i<header.length;i++){ const lower=(header[i]||"").toLowerCase();
        if (tableColsMap.has(lower)){ const exact=tableColsMap.get(lower);
          keep.push({ header: header[i], idx: i, target: exact,
            isArray: arrSet.has(exact.toLowerCase()),
            isBytes: byteSet.has(exact.toLowerCase()) }); } }
      if (!keep.length) { console.warn(`Skip: ${path.basename(file)} has no usable columns for ${target}.`); continue; }
      const filtered = await buildFilteredCsv({ rows: rows.slice(1), keep, encoding: [...byteSet].length ? "latin1" : "utf8" });
      const colList = keep.map(k => qid(k.target)).join(", ");
      console.log(`\n> COPY INTO ${target} COLUMNS: ${colList}`);
      const ok = execPSQLFile(NEW_URL, `\\COPY ${qid(target)} (${colList}) FROM ${qpath(filtered)} CSV HEADER`);
      try { fs.unlinkSync(filtered); } catch {}
