...
export class PerformanceTester {
  ...
  static async generatePerformanceReport(userId: string): Promise<string> {
    ...
    const avgDuration = testResults
      ...
      .reduce((acc, r) => acc + r.duration, 0) / testResults.filter(r => r.status === 'success').length
    
    const successRate = (testResults.filter(r => r.status === 'success').length / testResults.length) * 100
