...
export async function POST(request: NextRequest) {
  try {
    ...
    if (useBackupCode) {
      ...
      if (isValidCode) {
        const codeIndex = hashedCodes.findIndex(hashedCode => {
          const testHash = require('crypto').createHash('sha256').update(code.toUpperCase()).digest('hex')
      ...
      if (user.mfaMethod === 'authenticator' && user.mfaSecret) {
        ...
        return NextResponse.json({ 
          error: 'Email MFA login flow not implemented. Please use authenticator app or backup codes.' 
        }, { status: 400 })
      }
    }

    if (!isValidCode) {
      await logSecurityEvent(user.id, 'MFA_LOGIN_FAILED', { 
    ...
    await logSecurityEvent(user.id, 'MFA_LOGIN_SUCCESS', { 
      method: useBackupCode ? 'backup_code' : user.mfaMethod 
    }, request)
