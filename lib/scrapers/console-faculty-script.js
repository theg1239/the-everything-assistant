/**
 * A scraper for vit.ac.in/faculty that fetches and parses
 * the content of each individual department page to build a complete JSON.
 *
 * How it works:
 * 1.  It first scrapes the main faculty page to get the school/department structure.
 * 2.  It uses the `fetch` API to download the HTML of each department page directly.
 * 3.  The downloaded HTML is parsed into a temporary document using `DOMParser`.
 * 4.  It then scrapes the faculty data from this in-memory document.
 * 5.  This process is repeated for every department.
 * 6.  A status indicator is shown on the screen to provide feedback, as this process can take time.
 * 7.  Once complete, it logs the final JSON to the console and copies it to the clipboard.
 */
    ...
    for (const school of universityData) {
        for (const dept of school.departments) {
            ...
            try {
                ...
                console.error(`Could not scrape department "${dept.department}" from URL ${dept.url}. Error: ${error.message}`);
...
/**
 * Parses a department page's document to extract faculty details.
 * @param {Document} doc - The HTML document of the department page.
 * @returns {Array} - An array of faculty objects.
 */
...
// --- Execute the Scraper ---
// To run, paste the entire script into the browser console on https://vit.ac.in/academics/faculty
scrapeAllFaculty();