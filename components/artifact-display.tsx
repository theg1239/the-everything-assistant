...
import { FileSearch, ChevronDown, ChevronUp, ExternalLink, Maximize2, X, Users, Building2, GraduationCap, TrendingUp, Calendar, MapPin, Phone, Mail, Globe, UtensilsCrossed, Clock, User, Hash, BookOpen, BarChart3, CheckCircle, AlertTriangle, FileText, Sparkles, Info } from 'lucide-react'
...
const VTOPDataCard = ({ vtopData, onLoginClick }: { vtopData: any; onLoginClick?: () => void }) => {
    ...
    switch (command) {
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3">
                {validSubjects.map((subject: any, index: number) => {
                  ...
                  return (
                    <div key={index} className={`p-3 sm:p-4 rounded-lg border ${getStatusColor(percentage)}`}>
      ...
      case 'profile':
        ...
        if (profileData && Object.keys(profileData).length > 0) {
          ...
          const hiddenKeys = isMobile
            ? Object.keys(profileData).slice(5)
            : []
  ...
  return (
    <ResponsiveCard
      ...
      actions={(success !== false && !vtopData.requiresCredentials) ? (
  ...
  return (
    <Card className="w-full hover:shadow-sm transition-all duration-200 border-border bg-card">
      ...
      <CardContent className="pt-0 space-y-3">
        ...
        {isMobile && (paper.authors?.length > 2 || (paper.journal || paper.venue || paper.conference)?.length > 30) && (
          <Button
            variant="ghost"
            size="sm"
  ...
  return (
    <Card className="w-full hover:shadow-sm transition-all duration-200 border-border bg-card">
      ...
      <CardContent className="pt-0 space-y-3">
        <div className="space-y-2 text-xs text-muted-foreground">
          {company.description && (
            <p className={isMobile && !expanded ? "line-clamp-2" : ""}>
              {company.description}
            </p>
          )}
          {company.location && (
          ...
          return (
            <div key={mealType} className="space-y-2">
              ...
              <div className="ml-6 space-y-1">
                {itemsArray.slice(0, shouldCollapse && !isExpanded ? 3 : undefined).map((item: string, index: number) => (
        ...
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-border">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <Badge variant="secondary" className="hidden sm:flex">{itemCount} items</Badge>
            </div>
            <Button
              variant="ghost"
              size="icon"
