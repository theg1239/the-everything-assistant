        ...
        <div className="p-4 bg-gray-900 text-white rounded-lg font-sans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gray-800 p-4 rounded-lg space-y-4">
                    ...
                    {selectedCourse && (
                        <div className="mt-4">
                            <h4 className="font-bold text-lg">{selectedCourse.CODE} - {selectedCourse.TITLE}</h4>
                            ...
                            <div className="max-h-48 overflow-y-auto bg-gray-900 p-2 rounded-md">
                                {availableSlots.map((slot, index) => (
                                    <div key={`${slot.SLOT}-${index}`} className="p-2 border-b border-gray-700 flex justify-between items-center">
                                        <div>
                                            <p className="font-semibold">{slot.SLOT}</p>
                                            <p className="text-xs text-gray-400">{slot.FACULTY}</p>
                                        </div>
                                        <button onClick={() => addToTimetable(slot)} className="bg-green-600 hover:bg-green-700 rounded-md px-3 py-1 text-xs font-semibold">ADD</button>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="bg-gray-800 p-4 rounded-lg">
                    ...
                    <div className="h-96 overflow-y-auto bg-gray-900 p-2 rounded-md">
                        {timetable.length === 0 ? (
                            ...
                            timetable.map((entry) => (
                                <div key={entry.id} className="p-3 border-b border-gray-700 flex justify-between items-center">
                                    <div>
                                        <p className="font-bold">{entry.course.CODE} - {entry.course.TITLE}</p>
                                        <p className="text-sm text-gray-400">{entry.course.FACULTY} | <span className="font-semibold text-teal-400">{entry.course.SLOT}</span></p>
                                    </div>
                                    <button onClick={() => removeFromTimetable(entry.id)} className="bg-red-600 hover:bg-red-700 rounded-md px-3 py-1 text-xs font-semibold">REMOVE</button>
                                </div>
                            ))
                        )}
                    </div>
                ...
                <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
                    <div className="bg-gray-800 rounded-xl shadow-2xl w-full max-w-6xl h-full max-h-[90vh] flex flex-col">
                        <div className="flex justify-between items-center p-4 border-b border-gray-700">
                            ...
                            <button onClick={() => setShowTimetable(false)} className="text-gray-400 hover:text-white">
                                <X size={24} />
                            </button>
                        </div>
