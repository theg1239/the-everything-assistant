...
export function FeedbackSection() {
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      const response = await fetch('/api/feedback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'feedback', title: feedbackTitle, body: feedbackBody, user: session?.user }),
      })
  ...
  const renderContent = () => {
    switch (view) {
      case 'contribute':
        return (
          <motion.div key="contribute" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2 }}>
      ...
      case 'feedback':
        return (
          <motion.div key="feedback" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2 }}>
            <div className="flex items-center justify-between mb-4">
      ...
      default:
        return (
          <motion.div key="menu" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
