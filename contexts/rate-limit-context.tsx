...
export function RateLimitProvider({ children }: { children: React.ReactNode }) {
  ...
  const trySet = useCallback((info: Partial<RateLimitError>) => {
    ...
    if (!normalizedReset) {
      ...
      const overloadedHint = msg.includes('try again later') || msg.includes('overload') || msg.includes('overloaded') || msg.includes('temporarily')
      if (overloadedHint) {
        // Look for a numeric hint like '2 minutes' or '10 sec'
        const numericHint = msg.match(/(\d+)\s*(seconds|second|secs|sec|minutes|minute|mins|min|hours|hour|hrs|hr)/i)
        if (numericHint) {
          ...
          if (ms !== undefined) {
            ...
            try {
              ...
              console.debug('[RateLimit] numeric hint with unknown unit; skipping estimate:', numericHint[0])
            } catch {}
          }
        } else {
          try {
            // eslint-disable-next-line no-console
            console.debug('[RateLimit] overload hint found but no numeric hint; skipping estimated reset')
          } catch {}
        }
      }
    }
  ...
  const checkForRateLimitError = useCallback((error: any): boolean => {
    ...
    if (status === 429) {
      ...
      if (body && typeof body === 'string') {
        try {
          ...
          const numericRequestsPattern = /(\d+\s*requests?)\s*(per|\/)?\s*(second|sec|minute|min|hour|hr)/i
          const userPattern = /you have exceeded|exceeded your rate limit|user_rate_limit|requests?\s+per\s+(minute|hour|second)|requests?\/minute|per minute/i
          const userLimit = parsed.type === 'user_rate_limit' || /user/i.test(String(msg || '')) || numericRequestsPattern.test(String(msg || '')) || userPattern.test(String(msg || ''))
    ...
    const numericRequestsPattern2 = /(\d+\s*requests?)\s*(per|\/)?\s*(second|sec|minute|min|hour|hr)/i
    const isUserLimit = lower.includes('user') || lower.includes('you have exceeded') || numericRequestsPattern2.test(lower) || lower.includes('requests per')
          ...
          if (res.status === 429) {
            try {
              ...
              if (ct.includes('application/json')) {
                ...
                const raHeader = clone.headers.get('Retry-After') || clone.headers.get('retry-after') || clone.headers.get('x-rate-limit-reset') || clone.headers.get('x-ratelimit-reset')
                ...
                try {
                  ...
                  let reset = parsed.resetTime || parsed.reset_at || parsed.reset || parsed.retry_after
                  const raHeader = clone.headers.get('Retry-After') || clone.headers.get('retry-after') || clone.headers.get('x-rate-limit-reset') || clone.headers.get('x-ratelimit-reset')
                  if (!reset && raHeader) reset = raHeader
                  trySet({ resetTime: reset, userLimit: parsed.type === 'user_rate_limit', message: msg })
                  try {
                    // eslint-disable-next-line no-console
                    //console.debug('[RateLimit] parsed text 429 body JSON; reset sourced from', reset ? 'body/header' : 'none', 'reset=', reset)
                  } catch {}
                } catch {
                  const ra = res.headers.get('Retry-After') || res.headers.get('retry-after') || res.headers.get('x-rate-limit-reset') || res.headers.get('x-ratelimit-reset')
                  if (ra) {
                    const sec = parseInt(ra, 10)
                    if (!isNaN(sec)) {
                      trySet({ resetTime: new Date(Date.now() + sec * 1000).toISOString(), message: 'rate limit exceeded. please try again later.' })
                    } else {
                      trySet({ resetTime: ra, message: 'rate limit exceeded. please try again later.' })
                    }
                  } else {
                    trySet({ message: 'rate limit exceeded. please try again later.' })
                  }
