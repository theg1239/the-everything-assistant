...
export function getDecryptedVTOPCredentials(): { username: string; password: string } | null {
  try {
    ...
    const masterKey = getMasterKey()
    const decryptedPassword = CryptoJS.AES.decrypt(saved.encryptedPassword, masterKey).toString(CryptoJS.enc.Utf8)
...
export function getFormattedVTOPCredentials(): { username: string; encryptedPassword: string } | null {
  try {
