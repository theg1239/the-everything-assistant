...
import { getUser, createUser, getChats, getChatsWithMessageCounts, createChatWithFirstMessage, saveMessage } from '../lib/db'
...
async function seedTestData(userId: string, numChats: number = 5): Promise<void> {
  ...
  for (let i = 0; i < numChats; i++) {
    try {
      ...
      if (i % 3 === 0) {
        await saveMessage(chat.id, 'user', "Thanks! That's very helpful. Can you elaborate on the first point?")
        await saveMessage(chat.id, 'assistant', "Absolutely! Let me dive deeper into that concept. The key thing to understand is...")
      }
      
      if (i % 4 === 0) {
        await saveMessage(chat.id, 'user', "I implemented your suggestions. Here's what I found...")
        await saveMessage(chat.id, 'assistant', "Excellent progress! I can see you've applied the concepts well. For the next step, consider...")
        await saveMessage(chat.id, 'user', "Perfect, that makes sense. One more question about edge cases...")
        await saveMessage(chat.id, 'assistant', "Great question about edge cases! Here's how to handle those scenarios robustly...")
      }
...
async function runAdvancedPerformanceTest(userId: string): Promise<void> {
  ...
  try {
    ...
    for (const result of testResults) {
      ...
      console.log(`${statusIcon} ${perfIcon} ${result.operation}: ${result.duration}ms (${result.recordCount} records)`)
    }
    
    const successfulTests = testResults.filter(r => r.status === 'success')
    const avgDuration = successfulTests.reduce((acc, r) => acc + r.duration, 0) / successfulTests.length
