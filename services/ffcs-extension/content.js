(async () => {
    ...
    function getCsrfAndAuthId() {
      ...
      for (const script of scripts) {
        ...
        if (!authorizedId) {
          if (scriptContent.includes("getCoursesListForCurriculmCategory") || scriptContent.includes("getCoursesDetail")) {
      ...
      if (!csrfToken || !authorizedId) {
        ...
        throw new Error("CSRF token or Authorized ID not found. Ensure you are on the correct page and it's fully loaded.");
      }
    ...
    async function scrapeData() {
      ...
      try {
        ...
        for (let i = 0; i < categories.length; i++) {
          ...
          if (!courseSelect) {
            console.warn(`VTOP Scraper: No course list found for category ${category.value}. Skipping.`);
            continue;
          }
          ...
          for (let j = 0; j < courses.length; j++) {
            ...
            updateStatus(`Category ${i+1}/${categories.length}, Course ${j+1}/${courses.length}: ${course.code}`);
