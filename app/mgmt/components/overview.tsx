...
import { ComposedChart, Area, Line, ResponsiveContainer, Tooltip, XAxis, YAxis, CartesianGrid, ReferenceDot, Brush } from 'recharts'
...
export default function Overview({ stats, usage }: any) {
  ...
  const sparkData = useMemo(() => {
    ...
    if (usage?.lifetimeBuckets && Array.isArray(usage.lifetimeBuckets) && usage.lifetimeBuckets.length) {
      return usage.lifetimeBuckets.map((p:any) => ({ t: new Date(p.ts).toLocaleDateString(), ts: p.ts, v: p.totalTokens }))
    }
    // If caller provided an aggregated history field, use it.
    if (usage?.summaryAllTime && Array.isArray(usage.summaryAllTime)) {
      return usage.summaryAllTime.map((p: any) => ({ t: new Date(p.ts).toLocaleDateString(), v: p.totalTokens }))
    }
  ...
  const topEvents = useMemo(() => {
    if (!usage?.recent) return []
    return usage.recent.slice().sort((a:any,b:any)=>Number(b.totalTokens||0) - Number(a.totalTokens||0)).slice(0,5)
  }, [usage])

  // Derived statistics: avg, median, peak, 7-day trend (if we have daily buckets)
  const derived = useMemo(() => {
  const vals = (sparkData || []).map((d:any) => Number(d.v || 0)).filter((n:any)=>!Number.isNaN(n))
  ...
  const peakTs = peakIndex >= 0 ? sparkData[peakIndex].ts : undefined
    ...
    const last7Avg = last7.length ? last7.reduce((s:number,x:number)=>s+x,0)/last7.length : 0
    const prev7Avg = prev7.length ? prev7.reduce((s:number,x:number)=>s+x,0)/prev7.length : 0
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground lowercase">quick summary of system health and recent activity</p>
        </div>
        <div className="flex gap-4 items-center text-sm">
          <div className="text-muted-foreground lowercase">total tokens</div>
          <div className="font-semibold text-lg">{(usage?.summaryAllTime?.totalTokens ?? usage?.summary?.totalTokens)?.toLocaleString?.() ?? '—'}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 p-3 rounded-md bg-black/10">
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-muted-foreground lowercase">avg {derived.avg.toLocaleString()} · median {derived.median.toLocaleString()} · peak {derived.peak.toLocaleString()}</div>
          </div>
          <div style={{ width: '100%', height: 260 }}>
            <ResponsiveContainer>
              <ComposedChart data={sparkData} margin={{ top: 12, right: 16, left: 0, bottom: 12 }}>
                ...
                <Tooltip labelFormatter={(val:any) => new Date(val).toLocaleString()} formatter={(v:any) => [v?.toLocaleString?.() ?? v, 'tokens']} />
                <Area type="monotone" dataKey="v" stroke="#7C3AED" fillOpacity={0.08} fill="#7C3AED" />
                <Line type="monotone" dataKey="v" stroke="#A78BFA" strokeWidth={2} dot={false} />
                {derived.peakTs && <ReferenceDot x={derived.peakTs} y={derived.peak} r={4} fill="#F59E0B" stroke="none" />}
                {sparkData.length > 0 && <ReferenceDot x={sparkData[sparkData.length - 1].ts} y={sparkData[sparkData.length - 1].v} r={3} fill="#34D399" stroke="none" />}
                <Brush dataKey="ts" height={24} stroke="#475569" travellerWidth={8} tickFormatter={(val:any)=> new Date(val).toLocaleDateString()} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            {topEvents.map((e:any) => (
              <li key={e.id} className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-2 truncate text-xs" style={{ maxWidth: 160 }}>{(e.model || '-').toLowerCase()}</span>
                </div>
